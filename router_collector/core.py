...
class Capture:
    def __init__(self, root, provider, request, run_id):
        self.id = str(uuid.uuid4())
        self.path = private_dir(root / "active" / self.id)
        self.metadata = {"schema_version": 1, "id": self.id, "run_id": run_id,
        ...
        self.files = {key: (self.path / (key + ".bin")).open("wb") for key in self.hashes}
        for key in self.files:
            os.chmod(self.path / (key + ".bin"), 0o600)
    ...
    def close(self, root):
        ...
        self.metadata["bodies"] = {
            key: {"bytes": self.sizes[key], "sha256": self.hashes[key].hexdigest(),
                  "complete": self.complete[key]} for key in self.hashes}
        write_json(self.path / "metadata.json", self.metadata)
        self.path.rename(private_dir(root / "records") / self.id)


def create_app(provider, data_dir, run_id=None, *, _upstream=None):
    ...
    async def handle(request):
        capture = Capture(root, provider, request, run_id)
        response = None
        async def body():
            async for chunk in request.content.iter_chunked(65536):
                capture.append("request", chunk)
                yield chunk
        ...
        try:
            ...
            if response is not None and response.prepared:
                ...
                return response
            return web.json_response({"error": "upstream_or_capture_failure", "record_id": capture.id}, status=502)
        finally:
            capture.close(root)
    async def health(request):
        return web.json_response({"status": "ok", "provider": provider, "run_id": run_id, "version": "0.1.0"})
        ...
        if category == "records":
            for record in sorted(folder.iterdir()):
                if record.is_dir() and not record.is_symlink() and (record / "metadata.json").is_file():
                    files.extend(p for p in (record / "metadata.json", record / "request.bin", record / "response.bin") if p.is_file() and not p.is_symlink())
        else:
